"""
用纯字节方式生成所有前端文件，彻底清除 Unicode 零宽字符
"""
import os
...
:root {
  ...
  --accent: #FF6B6B; --accent-yellow: #FECA57;
  --success: #00D2D3; --bg-dark: #2C3E50;
...
with open(os.path.join(base, 'css', 'style.css'), 'wb') as f:
    f.write(css.strip())
...
const Storage = (() => {
  ...
  const SPORT_TYPES = [
    ...
    {id:'basketball', name:'\u7bee\u7403', icon:'\U0001f3c0', unit:'\u5206\u949f', unitLabel:'\u8fd0\u52a8\u65f6\u957f', dataField:'duration'},
    {id:'rope', name:'\u8df3\u7ef3', icon:'\U0001f3c3', unit:'\u6b21', unitLabel:'\u8df3\u7ef3\u6b21\u6570', dataField:'count'},
    {id:'running', name:'\u8dd1\u6b65', icon:'\U0001f3c3\u200d\u2642\ufe0f', unit:'km', unitLabel:'\u8dd1\u6b65\u8ddd\u79bb', dataField:'distance', extra:{unit:'\u5206\u949f',label:'\u7528\u65f6',field:'duration'}},
    {id:'swimming', name:'\u6e38\u6cf3', icon:'\U0001f3ca', unit:'\u7c73', unitLabel:'\u6e38\u6cf3\u8ddd\u79bb', dataField:'distance'},
    {id:'pushup', name:'\u4f8f\u5367\u6491', icon:'\U0001f4aa', unit:'\u4e2a', unitLabel:'\u5b8c\u6210\u4e2a\u6570', dataField:'count'},
    {id:'situp', name:'\u4ef5\u5367\u8d77\u5750', icon:'\U0001f9d8', unit:'\u4e2a', unitLabel:'\u5b8c\u6210\u4e2a\u6570', dataField:'count'},
    {id:'cycling', name:'\u9a91\u8f66', icon:'\U0001f6b4', unit:'km', unitLabel:'\u9a91\u884c\u8ddd\u79bb', dataField:'distance'},
    {id:'custom', name:'\u81ea\u5b9a\u4e49', icon:'\U0001f31f', unit:'', unitLabel:'\u8bf7\u8f93\u5165\u6570\u636e', dataField:'custom'}
  ];
  const PLANETS = [
    {id:'football', name:'\u8db3\u7403\u661f', icon:'\u26bd', color:'#00B894'},
    {id:'basketball', name:'\u7bee\u7403\u661f', icon:'\U0001f3c0', color:'#E17055'},
    {id:'rope', name:'\u8df3\u8dc3\u661f', icon:'\U0001f3c3', color:'#6C5CE7'},
    {id:'running', name:'\u8dd1\u6b65\u661f', icon:'\U0001f3c3\u200d\u2642\ufe0f', color:'#0984E3'},
    {id:'swimming', name:'\u6e38\u6cf3\u661f', icon:'\U0001f3ca', color:'#00CEC9'},
    {id:'strength', name:'\u529b\u91cf\u661f', icon:'\U0001f4aa', color:'#FDCB6E'}
  ];
  const BADGE_DEFS = [
    {id:'first_checkin', icon:'\U0001f947', name:'\u521d\u6b21\u51fa\u53d1', desc:'\u5b8c\u6210\u7b2c\u4e00\u6b21\u6253\u5361', condition:(records)=>records.length>=1},
    {id:'streak_3', icon:'\U0001f525', name:'\u4e09\u5929\u5c0f\u706b', desc:'\u8fde\u7eed\u6253\u5361 3 \u5929', condition:(_,user)=>user.streak>=3},
    {id:'streak_7', icon:'\U0001f525', name:'\u4e00\u5468\u706b\u7130', desc:'\u8fde\u7eed\u6253\u5361 7 \u5929', condition:(_,user)=>user.streak>=7},
    {id:'streak_30', icon:'\U0001f30b', name:'\u6708\u5ea6\u5f3a\u8005', desc:'\u8fde\u7eed\u6253\u5361 30 \u5929', condition:(_,user)=>user.streak>=30},
    {id:'sport_5', icon:'\U0001f31f', name:'\u5168\u80fd\u65b0\u661f', desc:'\u5b8c\u6210 5 \u79cd\u4e0d\u540c\u8fd0\u52a8', condition:(records)=>new Set(records.map(r=>r.sportId)).size>=5},
    {id:'total_20', icon:'\U0001f3c6', name:'\u8fd0\u52a8\u4e4b\u661f', desc:'\u7d2f\u8ba1\u6253\u5361 20 \u6b21', condition:(records)=>records.length>=20},
    {id:'total_50', icon:'\U0001f451', name:'\u8d85\u7ea7\u82f1\u96c4', desc:'\u7d2f\u8ba1\u6253\u5361 50 \u6b21', condition:(records)=>records.length>=50}
  ];
...
with open(os.path.join(base, 'js', 'storage.js'), 'wb') as f:
    f.write(js_storage.strip())
...
with open(os.path.join(base, 'js', 'avatar.js'), 'wb') as f:
    f.write(js_avatar.strip())
...
const Animation = (() => {
  function celebrate(count,colors){
    ...
    for(let i=0;i<count;i++){
      ...
      p.style.cssText='left:'+cx+'px;top:'+cy+'px;--dx:'+Math.cos(angle)*dist+'px;--dy:'+Math.sin(angle)*dist+'px;background:'+colors[i%colors.length]+';width:'+(6+Math.random()*8)+'px;height:'+(6+Math.random()*8)+'px;';
...
with open(os.path.join(base, 'js', 'animation.js'), 'wb') as f:
    f.write(js_animation.strip())
...
const App = (() => {
  ...
  function getSuccessMessage(sportId){
    ...
    return (msgs[sportId]||[])[\u0.Math.floor(Math.random()*2)]||'\u592a\u68d2\u4e86 \u7ee7\u7eed\u52a0\u6cb9\uff01';
  }
...
with open(os.path.join(base, 'js', 'app.js'), 'wb') as f:
    f.write(js_app.strip())
...
<body>
  <div id="page-splash" class="page active">
    <div class="splash-container">
      <div class="splash-mascot">\U0001f3c3</div>
  ...
  <div id="page-avatar-setup" class="page">
    <div class="container">
      <h2 class="page-title">\U0001f3a8 \u521b\u5efa\u4f60\u7684\u82f1\u96c4\u5f62\u8c61</h2>
      <div class="avatar-setup">
        ...
        <div class="avatar-actions"><button class="btn btn-secondary" onclick="document.getElementById('avatar-input').click()">\ud83d\udcf7 \u4e0a\u4f20\u7167\u7247</button></div>
        <div class="nickname-input"><label>\u4f60\u7684\u82f1\u96c4\u540d\u5b57\uff1a</label><input type="text" id="nickname-input" placeholder="\u8bf7\u8f93\u5165\u6635\u79f0" maxlength="10"></div>
        <button class="btn btn-primary btn-lg" onclick="App.finishSetup()">\u8fdb\u5165\u8fd0\u52a8\u661f\u7403 \U0001f680</button>
      </div>
    </div>
  </div>
  ...
  <div id="page-main" class="page">
    <header class="top-bar">
      ...
      <div class="stats-chip"><span>\U0001f525 \u8fde\u7eed <strong id="streak-days">0</strong> \u5929</span></div>
      <button class="btn btn-sm btn-accent" onclick="App.goToPage('page-checkin')">+ \u6253\u5361</button>
    </header>
    <div class="container map-container">
      <h2 class="section-title">\U0001f31f \u8fd0\u52a8\u661f\u7403\u5730\u56fe</h2>
    ...
    <nav class="bottom-nav">
      ...
      <button class="nav-btn" onclick="App.goToPage('page-achievements')">\U0001f3c6 \u52cb\u7ae0</button>
  ...
  <div id="page-checkin" class="page">
    <div class="container">
      ...
      <div class="checkin-photo" id="checkin-photo-area">
        <div class="photo-upload-area" id="photo-upload-area" onclick="document.getElementById('checkin-photo-input').click()"><span class="upload-icon">\ud83d\udcf7</span><span>\u70b9\u51fb\u4e0a\u4f20\u8fd0\u52a8\u7167\u7247</span></div>
      ...
      <div class="checkin-data-input" id="checkin-data-input" style="display:none">
        ...
        <div class="data-input-group"><input type="number" id="data-value" placeholder="\u8bf7\u8f93\u5165\u6570\u503c" min="0" step="any"><span class="data-unit" id="data-unit"></span></div>
        <div class="data-input-extra" id="data-input-extra" style="display:none"><input type="number" id="data-value-extra" placeholder="\u8bf7\u8f93\u5165\u989d\u5916\u6570\u503c" min="0" step="any"><span class="data-unit" id="data-unit-extra"></span></div>
      </div>
      <div class="checkin-note"><label>\u8fd0\u52a8\u611f\u60f3\uff08\u53ef\u9009\uff09\uff1a</label><textarea id="checkin-note" placeholder="\u4eca\u5929\u8fd0\u52a8\u611f\u89c9\u600e\u4e48\u6837\uff1f" maxlength="100"></textarea></div>
      <button class="btn btn-primary btn-lg btn-block" id="checkin-submit" onclick="App.submitCheckin()" disabled>\U0001f389 \u5b8c\u6210\u6253\u5361</button>
    </div>
  </div>
  ...
  <div id="page-achievements" class="page">
    <div class="container">
      <button class="btn-back" onclick="App.goToPage('page-main')">\u2190 \u8fd4\u56de</button>
      <h2 class="page-title">\U0001f3c6 \u6211\u7684\u52cb\u7ae0\u5899</h2>
  ...
  <div id="page-profile" class="page">
    <div class="container">
      ...
      <div class="profile-section"><h3>\ud83d\udcca \u8fd0\u52a8\u62a5\u544a</h3><div class="report-chart" id="report-chart"></div></div>
      <div class="profile-actions">
        <button class="btn btn-secondary" onclick="App.exportData()">\ud83d\udcde \u5bfc\u51fa\u6570\u636e</button>
        <button class="btn btn-danger" onclick="App.resetData()">\ud83d\uddbc\uufe0f \u91cd\u7f6e\u6570\u636e</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="modal-success" style="display:none">
    <div class="modal-content modal-success">
      <div class="success-animation" id="success-animation">\U0001f389</div>
      <h2>\u6253\u5361\u6210\u529f\uff01</h2>
      <p id="success-msg">\u592a\u68d2\u4e86\uff0c\u7ee7\u7eed\u52a0\u6cb9\uff01</p>
      <div class="badge-unlock" id="badge-unlock" style="display:none"><p>\U0001f3c6 \u89e3\u9501\u65b0\u52cb\u7ae0\uff01</p><span id="badge-icon">\U0001f947</span></div>
...
print('ALL DONE - all files regenerated cleanly')
